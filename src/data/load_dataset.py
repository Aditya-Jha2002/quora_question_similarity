...
def load_dataset(config_path):
    """ Runs data processing scripts to get the training data from (../initial) into
        raw data folder for further processing (saved in ../raw).
    """
    config = utils.read_params(config_path)
    shutil.copy(config["data_source"]["labeled_source"], config["load_dataset"]["raw_dataset_path"])
